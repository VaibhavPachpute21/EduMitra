  ...
  return (
    <div className="bg-body shadow d-flex flex-column flex-shrink-0 position-fixed top-0 bottom-0" style={{ width: '4.5rem' }}>
      ...
      <ul className="nav nav-pills flex-column text-center nav-flush mb-auto">
        ...
        {currentUser.user.role !==1 && <li className="nav-item">
          <Link className={`nav-link py-3 border-bottom rounded-0 ${location.pathname == "/Dashboard/PostedJobs/" || location.pathname ==  "/Dashboard/ViewJobs/" ? 'bg-terracotta text-white' : ''}`} to={currentUser.user.role===2? '/Dashboard/PostedJobs/':'/Dashboard/ViewJobs/'}>
