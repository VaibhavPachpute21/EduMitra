  ...
  return (
    <div className="bg-body shadow d-flex flex-column flex-shrink-0 position-fixed top-0 bottom-0" style={{ width: '4.5rem' }}>
      ...
      <ul className="nav nav-pills flex-column text-center nav-flush mb-auto">
        ...
        <li className="nav-item">
          <Link className={`nav-link py-3 border-bottom rounded-0 ${location.pathname == "/Dashboard/AddNewProject" || location.pathname == "/Dashboard/AddNewEvent" ? 'bg-terracotta text-white' : ''}`} to={currentUser.user.role == '0' ? "/Dashboard/AddNewProject" : "/Dashboard/AddNewEvent"} title={currentUser.user.role == '0'?'Add New Project':'Add New Event'}>
